    ...
    ADD overallAverage AS (
        ROUND(
            (techSupport + activeDirectory + projectManagement + helpDeskSupport + windowsServers +
             ...
             virtualization + coreCloudServices + apiUsage + httpResponseCodes + computationalThinking) / 15.0, 2
        )
    );
...
ALTER TABLE SoftwareDevRating
    ADD overallAverage AS (
        ROUND(
            (softwareEngineering + softwareDevelopmentLifecycle + programmingLanguages + dataStructuresAndAlgorithms +
