...
describe("ArticleStub", () => {
  ...
  it("renders the link with correct href", () => {
    ...
    const linkElement = screen.getByRole("link", {
      name: /Cick here to learn more/i,
  ...
  it("applies the correct class based on isPhotoFirst prop", () => {
    const { rerender } = render(
      <ArticleStub isPhotoFirst={true} imagesrc="test-image.jpg" />,
    );
    ...
    expect(containerElement?.className).toBe(
      "flex flex-row  justify-evenly flex-wrap gap-[42px] desktop:justify-evenly laptop:flex-nowrap",
    );
    ...
    expect(containerElement).toBeDefined();
    expect(containerElement?.className).toBe(
      "flex flex-row flex-row-reverse justify-evenly flex-wrap gap-[42px] desktop:justify-evenly laptop:flex-nowrap",
    );
  });
});
