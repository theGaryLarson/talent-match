...
export default function CreateJobseekerProfileIntroPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const birthDateValue =
      fields.find((f) => f.id === 'profile-creation-intro-birth-date')?.value ||
      null;
    const birthDateISO = birthDateValue
    ...
    const formData = {
      ...
      photoUrl:
        fields.find((f) => f.id === 'profile-creation-intro-avatar')?.value ||
        null,
      firstName:
      ...
      currentJobTitle:
        fields.find((f) => f.id === 'profile-creation-intro-current-position')
          ?.value || null,
      resumeUrl:
        fields.find((f) => f.id === 'profile-creation-intro-resume')?.value ||
        null,
    };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <fieldset>
            ...
            <div className="my-3 grid gap-6">
              <InputTextWithLabel
                type="date"
                id="profile-creation-intro-birth-date"
                onChange={handleFieldChange}
                value={
                  fields.find(
                    (f) => f.id === 'profile-creation-intro-birth-date',
                  )?.value || ''
                }
                required
              >
