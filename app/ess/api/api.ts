...
export const fetchPathways = async (): Promise<any> => {
    ...
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/cfa_pathwaies`, {
            method: 'GET',
            headers: {
...
export const fetchOccupations = async (pathway: string | undefined): Promise<any> => {

    try {
        const token = await getToken();
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/cfa_occupations?$filter=_cfa_pathway_value eq ${pathway}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json',
...
export const fetchOccupationRelatedData = async (occupationId: string): Promise<any> => {
    try {
        const token = await getToken();
...
export const fetchLightCastJobs = async (occupation_ONET_code: string | undefined): Promise<any> => {
...
export const fetchCareerBridgeCIPSOCS = async (soc_code: string | undefined): Promise<any> => {

    try {
        const token = await getToken();
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/cfa_careerbridgecipsocs?$select=cfa_careerbridgecipsocid,cfa_cipcode,cfa_soccode&$filter=(cfa_soccode eq '${soc_code}')`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: 'application/json',
...
export const fetchCareerBridgeITPrograms = async (cip_code: string | undefined): Promise<any> => {
