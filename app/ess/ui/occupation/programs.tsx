...
import { List, ListItem, Box, Typography, Link, Card, CardContent } from '@mui/material';
...
const Programs: React.FC<ProgramsProps> = ({ cipsocs }) => {
    ...
    useEffect(() => {
        const fetchPrograms = async () => {
            const programs = await Promise.all(cipsocs.map((bridge: any) => fetchCareerBridgeITPrograms(bridge.cfa_cipcode)))
                    ...
                    {itPrograms.map((program, index) => (
                        <ListItem key={index} alignItems="flex-start">
                            <Box>
                                ...
                                {program.cfa_completers && <Typography>
                                    Number of people who have completed the program: {program.cfa_completers}
                                </Typography>}

                                <Link display={"block"} component={NextLink} href={program.cfa_link} target="_blank" rel="noopener">
                                    View Program
                                </Link>
                            </Box>
                        </ListItem>
