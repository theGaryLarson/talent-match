...
export async function DELETE(request: Request, props: { params: Promise<{ certificateId: string }> }) {
    ...
    try {
        ...
        if (deletedEntry) { // Certificate was deleted
            return NextResponse.json({ success: true, result: deletedEntry });
        }
        else { // cert doesn't exist or jobseekerId mismatch
            return NextResponse.json({ error: `Failed to delete jobseeker certificate with id: ${certificateId}` });
        }
