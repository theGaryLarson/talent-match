...
export async function DELETE(request: Request, props: {params: Promise<{workId: string}>}) {
    ...
    try {
        ...
        if (deletedEntry) { // work experience was deleted
            return NextResponse.json({ success: true, result: deletedEntry });
        }
        else { // work experience doesn't exist or jobseekerId mismatch
            return NextResponse.json({ error: `Failed to delete jobseeker work experience with id: ${jsWorkExpId}` });
        }
    } catch (e: any) {
