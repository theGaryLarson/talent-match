...
export async function DELETE(request: Request, props: {params: Promise<{eduId: string}>}) {
    ...
    try {
        ...
        if (deletedEntry) { // education was deleted
            return NextResponse.json({ success: true, result: deletedEntry });
        }
        else { // education doesn't exist or jobseekerId mismatch
            return NextResponse.json({ error: `Failed to delete jobseeker education with id: ${eduId}` });
        }
    } catch (e: any) {
