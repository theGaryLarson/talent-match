...
export async function GET(request: Request, {params}: {params: {userId: string}}) {
    try {
        ...
        const jobseekerSkills: SkillDTO[] = data?.jobseeker_has_skills?.map(skill => skill.skills) || []
        const projectSkills: SkillDTO[] = data?.project_experiences?.flatMap(exp => exp.project_has_skills?.map(skill => skill.skills) || []) || [];

        // Create a Set of jobseeker skill IDs
        const jobseekerSkillIds = new Set(jobseekerSkills.map(skill => skill.skill_id));

        // Filter project skills to remove any that are already in jobseeker skills
        const uniqueProjectSkills: SkillDTO[] = projectSkills.filter(skill => !jobseekerSkillIds.has(skill.skill_id));
