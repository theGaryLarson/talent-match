'use client';
import { Alert, Box, Checkbox, FormControl, FormControlLabel, FormGroup, FormHelperText, FormLabel, Grid2, Link, List, ListItem, ListItemText, Paper, Snackbar, TextField, Typography } from "@mui/material";
...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const response = await fetch('/api/jobseekers/career-prep/enrollment/submit/', {
      method: 'POST',
      headers: {
    ...
    <>
      ...
      <Box className="flex justify-center">
        ...
        <List sx={{ listStyleType: 'disc', '& .MuiListItem-root': { display: 'list-item', marginLeft: 3 }, }}>
          <ListItem disablePadding><ListItemText>Women</ListItemText></ListItem>
          <ListItem disablePadding><ListItemText>Black, Latinx, Indigenous and Native American, Asian American, and Pacific Islander individuals</ListItemText></ListItem>
          <ListItem disablePadding><ListItemText>LGBTQIA2S+ individuals</ListItemText></ListItem>
          <ListItem disablePadding><ListItemText>Individuals with disabilities</ListItemText></ListItem>
          <ListItem disablePadding><ListItemText>Low-income individuals</ListItemText></ListItem>
          <ListItem disablePadding><ListItemText>Residents of underserved communities (e.g., Tribal Lands, Persistent Poverty Counties, rural areas)</ListItemText></ListItem>
        </List>
        <Typography sx={{ mb: 1 }}>
          To comply with grant requirements and ensure program effectiveness, we are collecting data on participant demographics.
          Your personal information is confidential and will be used solely for grant reporting purposes.
        </Typography>
        <Typography>
          For more information about the WJI and GJC Grant, visit:
        </Typography>
        <Link component={NextLink} target="_blank" href={'https://wsac.wa.gov/sites/default/files/Washington-Student-Achievement-Project-Narrative.pdf'}>https://wsac.wa.gov/sites/default/files/Washington-Student-Achievement-Project-Narrative.pdf</Link>
            ...
            <Grid2 container>
              ...
              <Grid2 size={12}>
                <FormControl required component='fieldset' sx={{ mb: 2 }}>
                  <FormLabel component="legend">Please indicate if you identify with any of the following priority populations:</FormLabel>
                  <FormHelperText sx={{m: 0}}>Select all that apply</FormHelperText>
                  <FormGroup>
                    {priorityPopulations.map((type) => (
                      <FormControlLabel
                        ...
                        control={
                          <Checkbox
                            checked={formData?.priorityPopulations.includes(type)}
