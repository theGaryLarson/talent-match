...
interface EmployerFeedbackFormProps {
    jobroles: (JobRole & { skills: (JobRoleSkill&{
        skill: {
            skill_name: string;
        };
    })[] })[];
}
export default function EmployerFeedbackForm({jobroles}:EmployerFeedbackFormProps){
    //fetch all ict roles
    // select a role
 ...
 const [selectedJobRole, setSelectedJobRole] = useState<(JobRole & { skills: (JobRoleSkill&{
        skill: {
            skill_name: string;
        };
    })[] }) | null>(null);
    const [skillRatings, setSkillRatings] = useState<{ [skillId: string]: number }>({});
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            {/* Job Role Selection */}
            <div>
                <label className="block font-medium text-gray-700">Select Job Role:</label>
                <select
            ...
            {selectedJobRole && (
                <div>
                    <h3 className="font-semibold text-gray-800">Rate the importance of each skill (1-5):</h3>
                    {selectedJobRole.skills.map((skill) => (
                        <div key={skill.skillId} className="flex items-center my-2">
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    handleSkillRatingChange(skill.skillId, parseInt(e.target.value))
                                }
                            >
            ...
            {selectedJobRole && (
                <div>
                    <label className="block font-medium text-gray-700">Projected Hires in the Next 3 Years:</label>
                    <input
                        type="number"
                        min="0"
                        className="w-full mt-2 p-2 border rounded-md"
