export default function GreyBackgroundBulletPointListBlock({title = "Technical Skills Development", 
                                                            ...
                                                            paragraph,
                                                            singleColumn = false} :
                                                            {title?: string, bulletPoints?: Array<any>, paragraph?: string, singleColumn?: boolean}){
    ...
    let buildBulletList = (bullets: Array<any>) => {
        ...
        for(let bullet of bullets){
            ...
            if(Array.isArray(bullet)){
                ...
                element = 
                (<li>
                    <p className="bg-blue-square bg-no-repeat bg-left pl-8">{bullet[0]}</p>
                    <ul className = "list-inside pl-8">{subBullets}</ul>
                </li>);
            } else {
                element = <li><p className="bg-blue-square bg-no-repeat bg-left pl-8">{bullet}</p></li>;
            }
            list.push(element);
        }
        return list;
