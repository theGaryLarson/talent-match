        ...
        <React.Fragment>
            <Box sx={{ display: 'flex', alignItems: 'center', textAlign: 'center' }}>
                <Tooltip title="Account settings">
                    ...
                    >
                        <p className={"hidden sm-tablet:contents flex items-center text-sm normal-case " + textColor}>{session?.user?.name || ''}</p>
                        <ChevronDownIcon className={"h-5 w-5 flex-none " + textColor} aria-hidden="true" />
                        <Avatar sx={{ width: 50, height: 50 }} alt={session?.user?.name || ''} src={session?.user?.image || ''} />
                    </Button>
                </Tooltip>
            </Box>
            <Menu
